...
async function getProjectCode(): Promise<void> {
    ...
    try {
        // 先获取代码信息
        const codeInfoRep = await axios.get(`/api/projects/${route.params.tpid}/code/`, {
            params: {
                temp: true,  // 标记项目为临时项目
            }
        });
        ...
        for (const cid in baseCodeInfo) {
            const codeRep = await axios.get(`/api/projects/${route.params.tpid}/code/${cid}`, {
                params: {
                    temp: true
                }
            });

            if (codeRep.status !== 200 || !codeRep.data || codeRep.data.code) {
                throw codeRep;
            }
...
function serverAddFile(fname: string) {
    ...
    // 和服务器沟通 新建文件
    axios.post(`/api/projects/${route.params.tpid}/code/`, {
        name: filename,
        type: filetype,
        temp: true
    })
...
function serverDeleteFile(fname: string) {
    ...
    // 而后删除项目
    axios.delete(`/api/projects/${route.params.tpid}/code/${cid}`, {
        data: {
            temp: true
        }
    })
...
async function saveChanges(): Promise<void> {
    ...
    for (const _cid in code.value) {
        ...
        const rep = await axios.patch(`/api/projects/${route.params.tpid}/code/${_cid}`, {
            content: codestr,
            temp: true
        });
