...
def copyTemporaryProjectAsProject(name: str) -> str:
    """
    将指定的工程文件夹复制为正式工程
    方法返回被复制为的临时工程的文件夹名
    src: str  源工程文件夹名
    ...
    srcProjectPath = os.path.join(TEMP_PROJECT_BASE_DIR, name)
    ...
    try:
        shutil.copytree(
            srcProjectPath,
            os.path.join(PROJECT_BASE_DIR, newTemporaryProjectName)
        )
